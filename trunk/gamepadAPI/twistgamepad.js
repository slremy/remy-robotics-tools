...
/*
 Code modified to provide twist messages
 S.L. Remy 12/12/12
 */
...
var gamepadSupport = {
  ...
  updateDisplay: function(gamepadId) {
	...
	if (gamepad.buttons[6] || gamepad.buttons[7]){
  	...
  	var now = new Date();
		local_command = "&lz="+(-1*gamepad.axes[1]).toFixed(3)+"&az="+(-1*gamepad.axes[0]).toFixed(3)+"&lx="+(-1*gamepad.axes[3]).toFixed(3)+"&ly="+(-1*gamepad.axes[2]).toFixed(3)+"&id="+gamepadId+"&time="+now.getTime();
