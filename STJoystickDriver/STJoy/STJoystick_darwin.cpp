  ...
  if (NULL == pDevice->interface)
  {
    ...
    if (kIOReturnSuccess == result)
    {
      ...
      if (S_OK != plugInResult)
        HIDReportError ((char*)"Couldn't query HID class device interface from plugInInterface");
      (*ppPlugInInterface)->Release (ppPlugInInterface);
    }
    else
      HIDReportError ((char*)"Failed to create **plugInInterface via IOCreatePlugInInterfaceForService.");
  }
  if (NULL != pDevice->interface)
  {
    result = (*(pDevice->interface))->open (pDevice->interface, 0);
    if (kIOReturnSuccess != result)
      HIDReportError ((char*)"Failed to open pDevice->interface via open.");
    else
...
static IOReturn HIDCloseReleaseInterface (recDevice *pDevice)
{
  ...
  if ((NULL != pDevice) && (NULL != pDevice->interface))
  {
    ...
    if (kIOReturnNotOpen == result)
    {
      ...
      HIDReportError ((char*)"Failed to close IOHIDDeviceInterface.");
    /* release the interface */
    result = (*(pDevice->interface))->Release (pDevice->interface);
    if (kIOReturnSuccess != result)
      HIDReportError ((char*)"Failed to release IOHIDDeviceInterface.");
...
static void HIDTopLevelElementHandler (const void * value, void * parameter)
{
  ...
  if (!CFNumberGetValue ((const __CFNumber*)refCF, kCFNumberLongType, &((recDevice *) parameter)->usagePage))
    HIDReportError ((char *)"CFNumberGetValue error retrieving pDevice->usagePage.");
  ...
  if (!CFNumberGetValue ((const __CFNumber*)refCF, kCFNumberLongType, &((recDevice *) parameter)->usage))
    HIDReportError ((char *)"CFNumberGetValue error retrieving pDevice->usage.");
}
    ...
    if (usbProperties)
    {
      ...
      if (refCF)
      {
        if (!CFStringGetCString ((const __CFString*)refCF, pDevice->product, 256, CFStringGetSystemEncoding ()))
          HIDReportError ((char *)"CFStringGetCString error retrieving pDevice->product.");
      }
      ...
      HIDReportError ((char *)"IORegistryEntryCreateCFProperties failed to create usbProperties.");

    if (kIOReturnSuccess != IOObjectRelease (parent2))
      HIDReportError ((char *)"IOObjectRelease error with parent2.");
    if (kIOReturnSuccess != IOObjectRelease (parent1))
      HIDReportError ((char *)"IOObjectRelease error with parent1.");
  }
}
...
recDevice *HIDDisposeDevice (recDevice **ppDevice)
{
  ...
  if (*ppDevice)
  {
    ...
    if (kIOReturnSuccess != result)
      HIDReportError ((char *)"HIDCloseReleaseInterface failed when trying to dipose device.");
